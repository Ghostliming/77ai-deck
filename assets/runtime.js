...
(function(){
  ...
  timerCount.textContent = (idx + 1) + ' / ' + total;
    ...
    /* ===== Image lightbox (click [data-lightbox]) ===== */
    let lightboxEl = null;
    ...
    function ensureLightbox() {
      ...
      lightboxEl.className = 'deck-lightbox';
      lightboxEl.setAttribute('role', 'dialog');
      lightboxEl.setAttribute('aria-modal', 'true');
      lightboxEl.hidden = true;
      lightboxEl.innerHTML =
        '<button type="button" class="deck-lightbox-backdrop" aria-label="关闭"></button>' +
        '<figure class="deck-lightbox-figure">' +
          '<button type="button" class="deck-lightbox-close" aria-label="关闭">×</button>' +
          '<img class="deck-lightbox-img" alt="">' +
          '<figcaption class="deck-lightbox-caption"></figcaption>' +
        '</figure>';
      document.body.appendChild(lightboxEl);
      lightboxEl.querySelector('.deck-lightbox-backdrop').addEventListener('click', closeLightbox);
      lightboxEl.querySelector('.deck-lightbox-close').addEventListener('click', closeLightbox);
      return lightboxEl;
    }
    ...
    function openLightbox(src, caption) {
      ...
      const img = lb.querySelector('.deck-lightbox-img');
      const cap = lb.querySelector('.deck-lightbox-caption');
      img.style.display = '';
      img.alt = caption || '';
      ...
      img.onerror = function () {
        ...
        cap.textContent = (caption || '演示截图') + ' · 图片待补充';
        cap.hidden = false;
      };
      img.src = src;
      lb.hidden = false;
      ...
      lightboxOpen = true;
      document.body.classList.add('deck-lightbox-active');
    }

    function closeLightbox() {
      if (!lightboxOpen || !lightboxEl) return false;
      lightboxEl.classList.remove('is-open');
      lightboxEl.hidden = true;
      lightboxOpen = false;
      document.body.classList.remove('deck-lightbox-active');
      const img = lightboxEl.querySelector('.deck-lightbox-img');
    ...
    deck.addEventListener('click', function (e) {
      ...
      openLightbox(src, btn.getAttribute('data-lightbox-caption') || '');
    });
